...
## 高速化の仕組み
このプラグインは、主に次の工夫でエンコードの待ち時間を減らしています。

- NVENC への入力を毎フレーム登録し直さず、D3D11 テクスチャを再利用して登録コストを削減
- H.265 は非同期エンコードをデフォルトにして、CPUの待ちを減らしパイプライン化
- 音声出力と動画出力の書き込み競合を減らすため、書き込みを専用スレッドへ集約

用途により安定性優先の同期エンコードも選べるようにしています。
